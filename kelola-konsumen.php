...
$DataperHal = 5;
$jml_data = count($users);
$jml_hal = ceil($jml_data / $DataperHal);
$halaman_aktif = (isset($_GET['halaman'])) ? $_GET['halaman'] : 1;
$awalData = ($DataperHal * $halaman_aktif) - $DataperHal;

$pengguna = $user->limitUser($awalData, $DataperHal);
...
<body>
    ...
    <aside id="logo-sidebar"
        ...
        C285.563,81.188,120.842,80.939,120.842,206.782z" />
                ...
                <li>
                    <button class="w-full flex items-center p-2 text-gray-900 rounded-lg bg-gray-100 group"
                        data-dropdown-toggle="dropdown">
                        <svg class="flex-shrink-0 w-5 h-5 transition duration-75 text-gray-900" aria-hidden="true"
                            xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                            <path
                                d="M14 2a3.963 3.963 0 0 0-1.4.267 6.439 6.439 0 0 1-1.331 6.638A4 4 0 1 0 14 2Zm1 9h-1.264A6.957 6.957 0 0 1 15 15v2a2.97 2.97 0 0 1-.184 1H19a1 1 0 0 0 1-1v-1a5.006 5.006 0 0 0-5-5ZM6.5 9a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9ZM8 10H5a5.006 5.006 0 0 0-5 5v2a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-2a5.006 5.006 0 0 0-5-5Z" />
                        </svg>
                        <span class="flex-1 ms-1 text-gray-900">Kelola pengguna</span>
                        <svg class="flex-shrink-0 w-9 h-9 transition duration-75 text-gray-900" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path fill="none" d="M0 0h24v24H0z" />
                                <path d="M12 15l-4.243-4.243 1.415-1.414L12 12.172l2.828-2.829 1.415 1.414z"
    ...
    <div class="sm:ml-64">
        <?= $alert ? $alert : ''; ?>
        <div class="pt-10 grid justify-center">
            <h1 class="my-4 text-4xl font-bold text-center text-cyan-700">Kelola Konsumen</h1>
            <div class="relative my-5 self-end">
                <a href="tambah-konsumen.php"
                    class="py-2 px-2 text-white text-sm rounded-md bg-cyan-700 hover:bg-cyan-600 focus:ring-2 focus:ring-cyan-500">tambah
                    konsumen</a>
            </div>
            <div class="relative overflow-auto sm:rounded-lg shadow-xl w-[1047px]">
                <table class="text-sm text-left text-gray-500 border w-[1044px]">
                    ...
                    <tbody>
                        <?php foreach ($pengguna as $user): ?>
                            <tr class="bg-white border-b text-center font-medium text-gray-900 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 max-w-[1044px]">
                                    <?= $user->alamat ?>
                                </td>
                                <td class="px-6 py-4">
                                    <?= $user->no_hp ?>
                                ...
                                <!-- <td class="px-6 py-4">
                                    <?= isset($user->poin) ? $user->poin : "-" ?>
                                </td> -->
                                <td class="px-6 py-4">
                                    <a href="edit-user.php?user_id=<?= $user->user_id ?>"
                                        class="font-medium text-white bg-cyan-700 hover:bg-cyan-600 py-1.5 px-2 rounded-md">Ubah</a>
                                    <a href="delete-profile.php?user_id=<?= $user->user_id ?>"
                                        class="font-medium text-white bg-red-500 hover:bg-red-400 py-1.5 px-2 rounded-md"
                                        onclick="return confirm('yakin ingin hapus akun dengan id=<?= $user->user_id ?>?')">Hapus</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div class="flex gap-x-2 ms-2 mt-2">

                <?php for ($i = 1; $i <= $jml_hal; $i++): ?>
                    <?php if ($i == $halaman_aktif): ?>
                        <a href="?halaman=<?= $i ?>" class="px-4 py-2 bg-cyan-700 text-white font-semibold rounded-md">
                            <?= $i; ?>
                        </a>
                    <?php else: ?>
                        <a href="?halaman=<?= $i ?>"
                            class="px-4 py-2 bg-cyan-700/80 text-white font-semibol rounded-md hover:bg-cyan-600/80">
                            <?= $i; ?>
                        </a>
                    <?php endif; ?>
                <?php endfor; ?>
