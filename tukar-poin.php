...
if (isset($_POST['tukar'])) {
    // var_dump($_POST);
    $reward_id = $_POST['jenisP'];
    $user_id = $data->user_id;
    $result = $poin->TukarPoin($user_id, $reward_id);
    if ($result == true) {
        $poinp = $poin -> getPoinHadiahById($reward_id);
        // var_dump($poinp->poin);
        $sisapoin = $data->poin - $poinp->poin;
        // var_dump($sisapoin);
        $tukar = $poin -> UpdatePoin($user_id,$sisapoin);
        if ($tukar === true) {
...
<body>
    <div class="absolute w-full">
        <img src="aset/bg-poinn.png" alt="bg" class="absolute -z-10 h-screen w-screen object-cover object-right md:object-center">
    ...
    <nav class="relative bg-transparent backdrop-filter shadow-lg">
            ...
            <div class="flex h-[100px] items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div class="hidden md:block">
                        <div class="ml-80 flex items-baseline space-x-10">
                            ...
                            <a href="home.html" class="hover:border-b-2 text-white px-3 py-2 text-md font-semibold">Beranda</a>
                            <div class="relative">
                                <button id="artikelDropdown" class="text-white hover:border-b-2 px-3 py-2 text-md font-semibold focus:outline-none" aria-haspopup="true">
                                    Artikel
                                </button>
                            ...
                            <a href="jemput-sampah.html" class="hover:border-b-2 text-white px-3 py-2 text-md font-semibold">Jemput
                                Sampah</a>
                            <a href="#" class="text-cyan-600 border-cyan-600 border-b-2 px-3 py-2 text-md font-semibold" aria-current="page">Poin</a>
                        </div>
                        <script>
                ...
                <div class="hidden md:block">
                    <div class="ml-4 flex items-center md:ml-6">
                        <button type="button" class="relative rounded-full p-2 text-white hover:text-white hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                        ...
                        <div class="relative ml-3">
                            <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" type="button" class="relative flex p-2 max-w-xs items-center rounded-full hover:bg-cyan-700 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" aria-expanded="false" aria-haspopup="true" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                                <span class="absolute -inset-1.5"></span>
                ...
                <div class="-mr-2 flex md:hidden">
                    <!-- Mobile menu button -->
                    <button type="button" class="relative inline-flex items-center justify-center rounded-md bg-gray-800 p-2 text-gray-200 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" aria-controls="mobile-menu" aria-expanded="false">
        ...
        <div class="hidden" id="mobile-menu">
            <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                ...
                <a href="#" class="text-gray-300 hover:bg-gray-800 hover:text-white block rounded-md px-3 py-2 text-base font-medium">Artikel</a>
                <a href="#" class="text-gray-300 hover:bg-gray-800 hover:text-white block rounded-md px-3 py-2 text-base font-medium">Jemput
                    Sampah</a>
                <a href="#" class="text-gray-300 hover:bg-gray-800 hover:text-white block rounded-md px-3 py-2 text-base font-medium">Poin</a>
            </div>
            <div class="border-t border-gray-700 pb-3 pt-4">
                ...
                <div class="mt-3 space-y-1 px-2">
                    <a href="#" class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Profil
                        Saya</a>
    ...
    <div class="relative grid justify-center h-fit mt-16 gap-y-4">
    <div class="p-10 w-full md:max-w-xl bg-white/90 rounded-xl justify-self-center">
            <h1 class="text-center text-cyan-700 font-bold text-4xl mb-5">Tukar Poin</h1>
            <h2 class="text-lg text-cyan-700 font-semibold">Poin saya : <?= $data->poin ?></h2>
            <form action="" method="post">
                <div class="my-6">

                    <label for="jenis" class="block mb-2 text-sm font-medium text-gray-900">Pilih jenis penukaran</label>
                    <select name="jenisP" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <?php foreach ($postrewards as $rewards) : ?>
                            <option value="<?= $rewards->id ?>"><?= $rewards->nama ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <?php if($data->poin >= $rewards->poin):?>
                    <button name="tukar" type="submit" class="text-white bg-cyan-700 hover:bg-cyan-600 focus:ring-2 focus:outline-none focus:ring-cyan-500 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2.5 text-center">Tukar</button>
                <?php else: ?>
                    <span>Poin Tidak Mencukupi</span>
                <?php endif; ?>
                <a href="poin.php" class="bg-gray-100 border-2 border-cyan-700 text-cyan-700 hover:bg-white focus:ring-2 focus:outline-none focus:ring-cyan-500 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2.5 text-center">Kembali</a>
            </form>
        </div>
        <table class="text-sm text-left text-gray-500 border w-fit shadow-xl rounded-lg">
            <thead class="text-xs text-center text-white uppercase bg-cyan-700">
                <tr>
            ...
            <tbody>
                <?php foreach ($postrewards as $user) : ?>
                    <tr class="bg-white border-b text-center font-medium text-gray-900 whitespace-nowrap">
