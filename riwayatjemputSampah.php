...
$user = new User();
$dataUser = $user->getEmail($email);
$hp = $dataUser->no_hp;
$jemputsampah = new JemputSampah();
$jemputssampah = $jemputsampah->getAllJemputSampah($hp);
...
<head>
    ...
    <title>Riwayat Jemput Sampah</title>
...
<body class="bg-white">
    ...
    <div class="relative">
        ...
        <div class="absolute inset-0 flex flex-col justify-center items-center px-4 text-center">
            <h1 class="text-white text-4xl md:text-4xl lg:text-5xl font-bold mb-4">Riwayat Jemput Sampah</h1>
            <p class="text-white text-lg md:text-xl font-semibold">Apakah Anda menemukan kesalahan dalam data formulir Anda atau ingin membatalkan pesanan?  
                <br>Anda dapat mengedit atau membatalkan data Anda di sini.<br> 
            <button class="justify-between mt-5">
                <a class="text-white bg-cyan-700 px-5 md:mx-2 text-center py-1.5 rounded-md hover:bg-cyan-600 font-bold"
                    href="#riwayat">Lihat Riwayat</a>
            </button>
        </div>
    </div>
    <!-- Riwayat Jemput Sampah -->
    <div class="max-w-6xl mx-auto bg-white p-8 rounded-md mt-7">
        <h2 class="text-4xl text-center font-bold text-cyan-900 sm:text-5xl mb-10" id="riwayat">Riwayat Jemput Sampah</h2>
            ...
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <?php foreach ($jemputssampah as $jemputsampah) : ?>
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4"><?= $jemputsampah->poin ?></td>
                        <td class="px-6 py-4">
                            <a href="detailjemputsampah.php?id=<?= $jemputsampah->id ?>" class="text-cyan-700 hover:underline">Klik disini</a>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex space-x-2 justify-center">
                                <a href="editjemputSampah.php?id=<?= $jemputsampah->id ?>" class="text-white bg-cyan-700 px-5 text-center py-1.5 rounded-md hover:bg-cyan-600 font-bold">Edit</a>
                                <button onclick="confirmDelete('<?= $jemputsampah->id ?>')" class="text-white bg-gray-400 px-5 text-center py-1.5 rounded-md hover:bg-cyan-600 font-bold">Batalkan</button>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<script>
    function confirmDelete(id) {
        ...
        if (result) {
            window.location.href = `deletejemputSampah.php?id=<?= $jemputsampah->id ?>`;
        }
    }
    // /setelahdieedit/user/delet
