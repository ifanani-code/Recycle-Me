...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET["id"])) {
    ...
    if ($data === false) {
        echo '<h1>Pos tidak ditemukan (404)</h1> <br> <a href="berita.php">Beranda</a>';
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_FILES['image']['size'] > 0) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $saveFile)) {
            ...
            $result = $berita->updateBeritaById($data->id, $data->title, $data->subtitle, $data->content, $saveFile);
            if ($result === true) {
        ...
        $result = $berita->updateBeritaById($data->id, $data->title, $data->subtitle, $data->content, $existingImage);
        if ($result === true) {
            header("Location: berita.php");
            exit;
        } else {
                        ...
                        <div class="sm:col-span-4">
                            ...
                            <div class="mt-2">
                                <div class="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 w-full">
                                    <input type="text" name="title" id="title" value="<?=$data->title?>" class="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" placeholder="Masukkan Judul"></div>
                            </div>
                        </div>
                        <div class="sm:col-span-4">
