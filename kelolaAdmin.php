...
$jemputssampah = $jemputsampah->getAllJemputSampahA();
...
<head>
    ...
    <title>Admin-Kelola Jemput Sampah</title>
...
<body class="bg-white">
    ...
    <aside id="logo-sidebar"
        ...
        C285.563,81.188,120.842,80.939,120.842,206.782z" />
                ...
                <li>
                    <button class="w-full flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group"
                        ...
                        <svg class="flex-shrink-0 w-7 h-7 text-gray-50 transition duration-75 group-hover:text-gray-900"
                        ...
                        <span class="flex-1 ms-2 text-gray-100 group-hover:text-gray-900 text-left">Kelola
                            Artikel</span>
                ...
                <li>
                    <a href="kelolaAdmin.php" class="w-full flex items-center p-2 text-gray-900 rounded-lg bg-white group-hover:text-gray-900">
                        <svg class="flex-shrink-0 w-6 h-6 text-gray-900 transition duration-75 group-hover:text-gray-50"
        ...
        l14.93-21.323c16.427,11.515,27.145,30.55,27.145,52.116C359.011,277.21,330.546,305.684,295.404,305.708z" />
                        ...
                        <span class="flex-1 ms-2 text-gray-900 group-hover:text-gray-100">Kelola jemput sampah</span>
                    </a>
                </li>
                <li>
    ...
    <div class="ms-64">
        <div class="flex justify-center items-center">
            ...
            <h1 class="text-cyan-700 font-bold text-4xl pt-10 pl-10">Data Jemput Sampah</h1>
                <div class="max-w-7xl mx-auto bg-white p-8 rounded-md mt-7">
                    <div class="overflow-x-auto">
                        <table class="centered-table w-full text-sm text-gray-500">
                            <thead class="text-xs text-white uppercase bg-cyan-800">
                                <tr>
                                    <th class="px-6 py-3 table-cell">id</th>
                                    <th class="px-6 py-3 table-cell">poin</th>
                                    <th class="px-6 py-3 table-cell">bukti</th>
                                    <th class="px-6 py-3 table-cell-wide">nama kurir</th>
                                    <th class="px-6 py-3 table-cell">nomor telepon</th>
                                    <th class="px-6 py-3 table-cell-wide">alamat</th>
                                    <th class="px-6 py-3 table-cell">tanggal</th>
                                    <th class="px-6 py-3 table-cell">waktu</th>
                                    <th class="px-6 py-3 table-cell">mitra</th>
                                    <th class="px-6 py-3 table-cell-wide">catatan</th>
                                    <th class="px-6 py-3 table-cell">kertas</th>
                                    <th class="px-6 py-3 table-cell">plastik</th>
                                    <th class="px-6 py-3 table-cell">kaca</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($jemputssampah as $jemputsampah) : ?>
                                    <tr class="bg-white border-b">
                                        <th class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->id) ?></th>
                                        <td class="px-6 py-4 table-cell">
                                        <?php if($jemputsampah->poin == 0): ?>
                                            <button class="text-white bg-cyan-700 px-5 md:mx-2 text-center py-1.5 rounded-md hover:bg-cyan-600 font-bold">
                                                <a href="admin-userPoin.php?id=<?= $jemputsampah->id ?>">Beri Poin</a>
                                            </button>
                                        <?php else: ?>
                                            <?= $jemputsampah->poin ?>
                                        <?php endif; ?>
                                        </td>
                                        <td class="px-6 py-4 table-cell"><img src="<?= $jemputsampah->gambar ?>"</td>
                                        <td class="px-6 py-4 table-cell-wide"><?= htmlspecialchars($jemputsampah->namakurir) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->nomorTelepon) ?></td>
                                        <td class="px-6 py-4 table-cell-wide"><?= htmlspecialchars($jemputsampah->alamat) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->tanggalPenjemputan) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->waktuPenjemputan) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->mitra) ?></td>
                                        <td class="px-6 py-4 table-cell-wide"><?= htmlspecialchars($jemputsampah->catatan) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->bskertas) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->bsplastik) ?></td>
                                        <td class="px-6 py-4 table-cell"><?= htmlspecialchars($jemputsampah->bskaca) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
